...
class TestRAGSystemQueryHandling:
    ...
    def test_query_basic_content_search(self, mock_rag_system):
        ...
        # Setup
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "MCP stands for Model Context Protocol."
        ...
        call_args = mock_rag_system.mock_ai_generator.generate_response_sequential.call_args
        assert "Answer this question about course materials: What is MCP?" in call_args[1]["query"]
        assert call_args[1]["tools"] == mock_rag_system.tool_manager.get_tool_definitions()
    ...
    def test_query_with_session_management(self, mock_rag_system):
        ...
        # Setup
        mock_rag_system.session_manager.get_conversation_history.return_value = "Previous: What is MCP?\nResponse: MCP is..."
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "MCP has three main components."
        
        # Execute
        response, sources = mock_rag_system.query("Tell me more about MCP components", session_id="test_session")
        
        # Verify
        assert response == "MCP has three main components."
        
        # Verify session manager calls
        mock_rag_system.session_manager.get_conversation_history.assert_called_once_with("test_session")
        ...
        call_args = mock_rag_system.mock_ai_generator.generate_response_sequential.call_args
        assert call_args[1]["conversation_history"] == "Previous: What is MCP?\nResponse: MCP is..."

    def test_query_without_session(self, mock_rag_system):
        """Test query without session ID"""
        # Setup
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "Test response"
    ...
    def test_query_tool_definitions_passed(self, mock_rag_system):
        ...
        # Setup
        expected_tools = [{"name": "search_course_content"}, {"name": "get_course_outline"}]
    ...
    def test_query_empty_sources(self, mock_rag_system):
        ...
        # Setup
        mock_rag_system.tool_manager.get_last_sources.return_value = []
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "General knowledge response"
...
class TestRAGSystemErrorHandling:
    ...
    def test_query_ai_generator_error(self, mock_rag_system):
        ...
        # Setup
        mock_rag_system.mock_ai_generator.generate_response_sequential.side_effect = Exception("API Error")
    ...
    def test_query_session_manager_error(self, mock_rag_system):
        ...
        # Setup
        mock_rag_system.session_manager.get_conversation_history.side_effect = Exception("Session Error")
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "Test response"
    ...
    def test_query_source_handling_error(self, mock_rag_system):
        ...
        # Setup
        mock_rag_system.tool_manager.get_last_sources.side_effect = Exception("Source Error")
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "Test response"
...
class TestRAGSystemContentTypes:
    ...
    def test_specific_course_content_query(self, mock_rag_system):
        ...
        # Setup
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "MCP servers handle data processing."
        mock_rag_system.tool_manager.get_last_sources.return_value = [
            {"text": "Introduction to MCP - Lesson 2", "link": "https://example.com/lesson2"}
        ]
...
- Lesson 2: MCP Architecture"""
    ...
    def test_general_knowledge_query(self, mock_rag_system):
        ...
        mock_rag_system.mock_ai_generator.generate_response_sequential.return_value = "Machine learning is a subset of AI."
        mock_rag_system.tool_manager.get_last_sources.return_value = []  # No course sources
    ...
    def test_complex_multi_course_query(self, mock_rag_system):
        ...
        mock_rag_system.tool_manager.get_last_sources.return_value = [
            ...
            {"text": "API Design Course - Lesson 3", "link": "https://example.com/api/lesson3"}
        ]
...
class TestRAGSystemIntegration:
    ...
    def test_full_workflow_with_real_components(self, test_config):
        ...
        with patch('rag_system.DocumentProcessor') as mock_doc_processor, \
            ...
            mock_ai_generator.generate_response_sequential.return_value = "Test AI response"
